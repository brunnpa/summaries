...
/* Konstanten definieren                                                 */
   ...
   const char* DefaultPayload = "GET / http/1.0\nAccept: */*\nUser-Agent: client_www\n\n";
...
int main(int ArgumentCount, char* ArgumentValue[]) {
   ...
   */
   ...
   if (ArgumentCount > 3) {            /* Falls Payload-Parameter angegeben */
      Payload = ArgumentValue[3];   
   } else {
      Payload = DefaultPayload;               /* Default verwenden */
   }
   ...
   Status = send(CommunicationSocket, Payload, strlen(Payload) + 1, 0);
   ...
   while (CharsReceived > 0) {
      ...
      int count;
      ioctl(CommunicationSocket, FIONREAD, &count);
      if (count < 1) break;
      CharsReceived = recv(CommunicationSocket, Buffer, sizeof(Buffer), 0);
   }
