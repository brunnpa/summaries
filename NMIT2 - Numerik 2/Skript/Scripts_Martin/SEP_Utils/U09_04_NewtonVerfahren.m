...
% Jacobi-Matrix f�r die Funktion erstellen
...
% Funktionen f�r Vektoren �ffnen --EDIT (so viele wie oben Parameter)
DfVec = @(x) Df(x(1), x(2), x(3), x(4));
fVec = @(x) f(x(1), x(2), x(3), x(4));

% L�sung steht zum Schluss in xn
xn = lambda0;
err = tol + 1;
...
% F�r diese Aufgabe wird es zwischen den beiden Verfahren keine
% Unterschiede geben
